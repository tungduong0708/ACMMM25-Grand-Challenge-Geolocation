...
def get_gps_from_location(
    ...
    try:
        ...
        logger.error(f"Failed to parse geocoding response for '{location}': {parse_err}")
...
async def handle_async_api_call_with_retry(
    ...
    Args:
        api_call_func (Callable): An async function that returns a response.
        max_retries (int): Maximum retry attempts.
        base_delay (float): Initial delay for backoff (doubles each retry).
        fallback_result (Optional[dict]): Optional result to return on failure.
        error_context (str): Contextual info for logging.

    Returns:
        dict: API response or fallback result.
    """
