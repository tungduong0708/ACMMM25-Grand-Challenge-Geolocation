...
from g3.utils.dataset import MP16Dataset, im2gps3kDataset, yfcc4kDataset
...
class ZeroShotPredictor(nn.Module):
    ...
    def predict_image(self, image_path, top_k):
        ...
        image = self.model.vision_processor(images=image, return_tensors='pt')['pixel_values'].reshape(-1,224,224)
        image = image.unsqueeze(0).to(self.device) 
        gps_gallery = self.gps_gallery.to(self.device)  # [N, 768*3]

        with torch.no_grad():
            # Compute similarity (logits) and top-k
            logits_per_image = self.forward(image, gps_gallery)  # [B, N]
            ...
            top_pred_gps = self.gps_gallery[top_preds.indices]  # [B, k, 768*3]
            top_pred_prob = top_preds.values  # [B, k]                  # [top_k]

        return top_pred_gps, top_pred_prob
    
    def predict_im2gps3k_dataset(self, top_k, root_path='data/im2gps3k', image_data_path='im2gps3ktest'):
        self.model.eval()

        # Prepare dataset and dataloader
        dataset = im2gps3kDataset(vision_processor=self.model.vision_processor, text_processor=None, root_path=root_path, image_data_path=image_data_path)
    ...
    def evaluate_im2gps3k(self, df_path, top_k=5, root_path='data/im2gps3k', image_data_path='filtered_im2gps3k', text_data_path='im2gps3k_places365.csv'):
        """
        ...
        control_lons, control_lats = generate_control_locations(1000000)
        gps_gallery = torch.tensor(np.stack([control_lons, control_lats], axis=1)).to(self.device)
        all_topk_gps = []
        all_topk_probs = []
        ...
        with open("threshold_accuracy.txt", "a") as f:
            for threshold in [2500, 750, 200, 25, 1]:
                ...
                res.append(acc)
                line = f"{threshold}km level: {acc:.4f}\n"
                f.write(line)
                print(line.strip())
...
def main():
    ...
    predictor = ZeroShotPredictor(
        model='checkpoints/mercator_finetune_weight.pth',  # Path to your model
    ...
    top_pred_gps, top_pred_prob = predictor.predict_image(
        image_path="C:/Users/tungd/Desktop/FR8QTCSX0AA_fIW.jpg",  # Path to your test image
        top_k=5  # Number of top predictions to return
    )
    print(top_pred_gps)
    print(top_pred_prob)
